...
class DataExportImport:
    ...
    def export_import_data(self):
        ...
        tk.Label(self.root, text="Data Export/Import", font=("Times New Roman", 18)).pack(pady=10)
    ...
    def export_parking_records(self):
        ...
        if self.parking_records == {}:
            ...
            return
        try:
            df = pd.DataFrame(self.parking_records, columns=["License Plate", "Entry Time"])
            file_path = filedialog.asksaveasfilename(defaultextension=".xlsx", 
                                                    filetypes=[("Excel Files", "*.xlsx")])
            if file_path:
                df.to_excel(file_path, index=False)
                messagebox.showinfo("Success", "Parking records exported successfully.")
        except Exception as e:
            messagebox.showerror("Error", f"Failed to export parking records: {e}")
    ...
    def export_history_records(self):
        ...
        if self.history_records == []:
            ...
            return
        try:
            df = pd.DataFrame(self.history_records, columns=["License Plate", "Entry Time", "Exit Time", "Fee"])
            file_path = filedialog.asksaveasfilename(defaultextension=".xlsx", 
                                                    filetypes=[("Excel Files", "*.xlsx")])
            if file_path:
                df.to_excel(file_path, index=False)
                messagebox.showinfo("Success", "History records exported successfully.")
        except Exception as e:
            messagebox.showerror("Error", f"Failed to export history records: {e}")
    ...
    def import_records(self, record_type):
        ...
        try:
            ...
            if file_path:
                ...
                messagebox.showinfo("Success", f"Records imported successfully into {record_type} records.")
        except Exception as e:
